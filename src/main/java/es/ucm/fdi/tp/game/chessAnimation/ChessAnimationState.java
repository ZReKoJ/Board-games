...
import static es.ucm.fdi.tp.game.chessAnimation.ChessAnimationBoard.DIM;
import static es.ucm.fdi.tp.game.chessAnimation.ChessAnimationBoard.empty;
import static es.ucm.fdi.tp.game.chessAnimation.ChessAnimationBoard.enemy;
import static es.ucm.fdi.tp.game.chessAnimation.ChessAnimationBoard.sameTurn;
...
public class ChessAnimationState extends GameState<ChessAnimationState, ChessAnimationAction> {

    private static final Logger log = Logger.getLogger(ChessAnimationState.class.getName());

    private final int turn;
    private boolean finished;
    private int winner;
    private final ChessAnimationBoard board;
    ...
    protected ArrayList<ChessAnimationAction> valid;

    /**
     * Creates an empty chess board;
     */
    public ChessAnimationState() {
        super(2);
        turn = WHITE;
        winner = -1;
        finished = false;
        board = new ChessAnimationBoard();
        canCastle = new int[]{
     ...
     */
    public ChessAnimationState(ChessAnimationState previous, ChessAnimationBoard board,
    ...
    public boolean isValid(ChessAnimationAction action) {
     ...
     */
    protected static boolean findKing(ChessAnimationBoard board, int turn, Point kingPos) {
        for (int i = 0; i < DIM; i++) {
            for (int j = 0; j < DIM; j++) {
    ...
    private void updateValid() {
        ...
        valid = new ArrayList<>();
        ArrayList<ChessAnimationAction> candidates = new ArrayList<>();
        ...
        for (ChessAnimationAction a : candidates) {
            ChessAnimationBoard next = new ChessAnimationBoard(board);
            a.applyTo(next);
     ...
     * @return
     */
    public List<ChessAnimationAction> validActions(int playerNumber) {
        return valid;
    }
    ...
    public double evaluate(int turn) {
        ...
        for (int i = 0; i < DIM; i++) {
            for (int j = 0; j < DIM; j++) {
                byte p = board.get(i, j);
                if (!ChessAnimationBoard.empty(p)) {
     ...
     */
    public ChessAnimationBoard getBoard() {
        return new ChessAnimationBoard(board);
    }
    ...
    protected void addPawnAction(int turn, int row, int col,
                                 ...
                                 ArrayList<ChessAnimationAction> as) {
        if (row != queenRow) {
            as.add(new ChessAnimationAction(turn, row, col, dstRow, dstCol));
        } else {
            for (Special s : ChessAnimationAction.QUEENING_SPECIALS) {
                as.add(new ChessAnimationAction(turn, row, col, dstRow, dstCol, s));
            }
        }
    }

    protected void pawnActions(byte p, int turn,
                               int row, int col, ArrayList<ChessAnimationAction> as,
        ...
        if (empty(board.get(row + dy, col))) {
            ...
            if (row == doubleRow && empty(board.get(row + dy + dy, col))) {
                as.add(new ChessAnimationAction(turn, row, col, row + dy + dy, col));
            }
        }
        ...
        if (row == passantRow && enPassant != -1) {
            if (enPassant == col + 1) {
                as.add(new ChessAnimationAction(turn, row, col,
    ...
    protected void deltaActions(byte p, int turn,
                                int row, int col, int dy, int dx, ArrayList<ChessAnimationAction> as,
                                boolean multiple) {
        ...
        do {
            ...
            if (empty(target) || enemy(p, target)) {
                as.add(new ChessAnimationAction(turn, row, col, dRow, dCol));
            }
        } while (multiple && empty(target));
    }

    protected static boolean canDeltaCapture(ChessAnimationBoard b, byte p, int row, int col, int dy, int dx,
    ...
    protected void generateActions(byte p, int turn,
        ...
        if (piece == Piece.Pawn) {
            if (ChessAnimationBoard.black(p)) {
     ...
     */
    protected static boolean threatenedBy(ChessAnimationBoard b, int turn, int row, int col) {
        for (int i = 0; i < DIM; i++) {
            for (int j = 0; j < DIM; j++) {
                byte p = b.get(i, j);
    ...
    protected static boolean canCapture(ChessAnimationBoard b, byte p, int row, int col, int dstRow, int dstCol) {
        Piece piece = Piece.valueOf(p);
        if (piece == Piece.Pawn) {
            return pawnCanCapture(p, row, col, dstRow, dstCol,
                    ChessAnimationBoard.black(p) ? 1 : -1);
        } else if (piece.getDeltas() != null) {
            for (int[] d : piece.getDeltas()) {
