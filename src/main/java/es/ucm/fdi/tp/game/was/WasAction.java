...
public class WasAction implements GameAction<WasState, WasAction> {
    ...
    public WasAction(int player, int row_ini, int col_ini, int row_fin, int col_fin) {
    ...
    public WasState applyTo(WasState state) {
        ...
        board[this.row_ini][this.col_ini] = WasFigures.BLACK.getRepresentation();
        ...
        if (WasState.isWinner(board, state.getTurn())) {
            ...
            next = new WasState(state.getTurn(), board, true);
        } 
        else {
        	// aun no ha ganado ninguno de los dos
            next = new WasState(state.getTurn(), board, false);
        }
