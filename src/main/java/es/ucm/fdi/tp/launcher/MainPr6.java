...
public class MainPr6 {
		...
		if(Games.find(gType)){
			for(int i = 0; i < playerModes.length; ++i){
				if(playerModes[i].equalsIgnoreCase("manual"))
					...
					players.add(new SmartPlayer("player " + i, game.getSmartDepth()));
				else{
	...
	void startGUIMode(final String gType, Games game, String playerModes[]) {
		...
		for (int i = 0; i < game.getState().getPlayerCount(); ++i){
			final GamePlayer rand;
			final ConcurrentAiPlayer smart;
			rand = new RandomPlayer("Player - " + i);
			smart = new ConcurrentAiPlayer("Player - " + i);
			rand.join(i);
			smart.join(i);
			final int id = i;
			String mode;
