package es.ucm.fdi.tp.extra.swing.Hebras;// Ejemplo de programación "thread-UNsafe"
...
public class EjemploUnsafe {
	...
	static class ItemImporte {
		...
		public ItemImporte(int imp) { importe = imp; next = null; }
	}
	
	static class Contabilidad {
		private volatile ItemImporte primero = null;
		private volatile ItemImporte ultimo = null;
		
		public void add(int importe)  {  /* NO ES synchronized */ 
			if (primero == null) {
		...
		public int numImportes() {
		  int len = 0;
		  ItemImporte curr = this.primero;
		  while (curr != null) {
		    len++;
		    curr = curr.next;
		  }
	...
	static class ClienteOnline extends Thread {
		...
		public ClienteOnline(String id, Contabilidad s) {this.id=id; ct=s;}
